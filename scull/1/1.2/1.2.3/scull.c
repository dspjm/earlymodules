...
static struct quantum *follow_quantums(struct quantum *head, int cnt)
{
	int i;
...
static struct quantum *add_quantums(struct scull_dev *sd, int cnt)
{
	int i;
	struct quantum *tmp1;
	struct quantum *tmp3;
							...
							loff_t *f_off)
{
	...
	kill_fasync(&tmp1->faq, SIGIO, POLL_IN);
	result = tmp7 - tmp8;
	tmp1->sum_len += result;
	tmp4->len += result;
	*f_off += result;
...
static int scull_fasync(int fd, struct file *filp, int mode)
{
	struct scull_dev *tmp1;
	tmp1 = filp->private_data;
	return fasync_helper(fd, filp, mode, &tmp1->faq);
}

static loff_t scull_llseek(struct file *filp, loff_t off, int whence)
{
	struct scull_dev *tmp1 = filp->private_data;
	switch (whence) {
	case 0:
	case SEEK_SET:
		tmp2 = off;
