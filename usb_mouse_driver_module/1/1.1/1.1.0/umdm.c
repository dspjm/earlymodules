...
enum umdm_cdev_ioctl_num {
	UMDM_USB_STD_REQ = _IOR(UMDM_CDEV_MAG, 1, int),
	UMDM_USB_READ_MOVEMENT = _IOW(UMDM_CDEV_MAG, 2, int),
};
...
/*prototype changed within >3.0 kernel*/
static long umdm_cdev_ioctl(struct inode *inode, struct file *filp, 
					unsigned int cmd, unsigned long arg)
{
	switch(cmd) {
	case UMDM_USB_STD_REQ:
		break;
	case UMDM_USB_READ_MOVEMENT:
		tmp1 = umdm_intfs.inftps[filp->private->intfs_index];
